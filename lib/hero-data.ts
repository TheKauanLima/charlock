...
const HERO_ASSET_SLUGS: Record<string, string> = {
...
function createHeroInfo(assetSlug: string, heroIndex: number): HeroInfoDefinition {
  const theme = HERO_INFO_THEMES[heroIndex % HERO_INFO_THEMES.length]
  const [tag1Tilt, tag2Tilt, tag3Tilt] = buildUniqueTilts(assetSlug)

  return {
    nameType: 'image',
    nameValue: `/panorama/images/heroes/hero_names/${assetSlug}.svg`,
    ...
    tag2Tilt,
    tag3Tilt,
    ability1Icon: `/panorama/images/hud/abilities/${assetSlug}/1.png`,
    ability2Icon: `/panorama/images/hud/abilities/${assetSlug}/2.png`,
    ability3Icon: `/panorama/images/hud/abilities/${assetSlug}/3.png`,
    ability4Icon: `/panorama/images/hud/abilities/${assetSlug}/4.png`,
...
function createHeroDefinition(hero: HeroSeed, heroIndex: number): HeroDefinition {
  const assetSlug = hero.assetSlug ?? HERO_ASSET_SLUGS[hero.slug] ?? hero.slug

  return {
    ...hero,
    assetSlug,
    heroInfo: createHeroInfo(assetSlug, heroIndex),
  }
}
